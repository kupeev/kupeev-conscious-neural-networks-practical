...
from conn_aux import get_info_for_save
...
#randomy creates png with observed/seen for train data
#under eu_op == 'op2'
#(in diffrence to make_joint_examples__test
#doen not use data__make_examples.npy)
make_joint_examples__train = 0
if make_joint_examples__train:
    am_exs = 10
    di_examples_train = 'joint_examples'
    project_dir = os.getcwd()  # Get the current working directory
    di_examples = os.path.join(project_dir, di_examples_train)
    if not os.path.exists(di_examples):
        os.makedirs(di_examples)

#creation observed/seen for test data
#
# >>> for R8 (nR==3)
#
#when for stoch
#   using data__make_examples.npy created under make_examples=1 in aside2.py,
#   under eu_op == 'op3'
#   creates png with observed/seen
#when for vanilla
#   as above stoch: for data__make_examples.npy made fore stcoh
#   run as for stoch with hconn but with hconn_simulatenonhconn
#   (dbg15 = 0)
make_joint_examples__test = 1
if make_joint_examples__test:

    conntype_4_make_joint_examples__test = 'vanilla' # 'stoch' or 'vanilla'
    # if 'stock', cteates in <joint_exampoles> files like
    #   nimabs=171_gt1_MO4_MS1_01_obs.png
    #   see get_info_for_save() for the notation
    #
    #   set dbg15 == 1 and hconn_simulatenonhconn == 0
    #
    # if 'vanilla', uses nimabs's previouly made in <joint_exampoles> for 'stock'
    #   build in a new <joint_exampoles> files like
    #   nimabs=171_gt1_MO4_MS1_obs.png
    #
    #   set dbg15 == 0 and hconn_simulatenonhconn == 1

    if conntype_4_make_joint_examples__test == 'vanilla':
        #filled by hand ising nimabs in <joint_examples?
        #cretaed previously for 'stoch' version
        list_nimabs_vanilla = [75,127,157,171,254,276,294,346,396,436,447,449,470,533,684,686,700,863,936,950]
    else:
        list_nimabs_vanilla = []

    #if not Note, just goiside to a specific nimabs
    #(this nimabs is among the correspomding to
    #pmg imgage, eg nimabs=296_gt5_MO0_MS5_01_seen)
    zoomin_nimabs = None#157 # normally None

    # zoomin_nimabs != None ==> conntype_4_make_joint_examples__test == 'stoch'
    assert zoomin_nimabs == None or conntype_4_make_joint_examples__test == 'stoch'
    ...
    '''
    am_per_case = 5
    di_examples_test = 'joint_examples'
    project_dir = os.getcwd()  # Get the current working directory
    di_examples = os.path.join(project_dir, di_examples_test)
    if not os.path.exists(di_examples):
        os.makedirs(di_examples)
    data__make_examples = np.load('data__make_examples.npy', allow_pickle=True).item()
...
tr_data_mt = 1  # use val data for model tuning, besides the standard train
# for 'op1':
#   load ready model
#   new model at each epoch saves to fn with _mt
all_train_in_mt_mode = 1
assert (not all_train_in_mt_mode) or tr_data_mt
assert not dbg24 or eu_op in ['op2', 'op3']
...
if dbg14:
    epochs = 5
elif tr_data_mt and not all_train_in_mt_mode:
    epochs = 10000
...
hconn = 1  # ('op2', 'op3', 'op2op3') means stochastic
if hconn:
    aug_h = 0
    hconn_simulatenonhconn = 1
...
if is_win:
    ...
    if tr_data_mt and not all_train_in_mt_mode:  # where from the models are taken
        # no sense meanwhile
        tr_data_mt_models = [
    ...
    model_fn_op2op3_s = [ \
            r'/home/research/iterative-semiotics-networks/models/model_rmnist_5aug0_Ep100000_p1/epoch96786_mt.pth',
            r'/home/research/iterative-semiotics-networks/models/model_rmnist_6aug0_Ep100000_p1/epoch80762_mt.pth',
            r'/home/research/iterative-semiotics-networks/models/model_rmnist_7aug0_Ep100000_p1/epoch88178_mt.pth',
            r'/home/research/iterative-semiotics-networks/models/model_rmnist_8aug0_Ep100000_p1/epoch98809_mt.pth',
            r'/home/research/iterative-semiotics-networks/models/model_rmnist_9aug0_Ep100000_p1/epoch99135_mt.pth',
            r'/home/research/iterative-semiotics-networks/models/model_rmnist_10aug0_Ep100000_p1/epoch88055_mt.pth',
            r'/home/research/iterative-semiotics-networks/models/model_rmnist_20aug0_Ep100000_p1/epoch53929_mt.pth',
            r'/home/research/iterative-semiotics-networks/models/model_rmnist_30aug0_Ep100000_p1/epoch99566_mt.pth',
            r'/home/research/iterative-semiotics-networks/models/model_rmnist_40aug0_Ep100000_p1/epoch93256_mt.pth',
            r'/home/research/iterative-semiotics-networks/models/model_rmnist_50aug0_Ep100000_p1/epoch90490_mt.pth',
    ]

    if tr_data_mt and not all_train_in_mt_mode:  # where from the models are taken
        tr_data_mt_models = [
            r'/home/research/projects2/iterative-semiotics-networks/models/model_rmnist_5aug6_Ep100000_p1.pth',
            r'/home/research/projects2/iterative-semiotics-networks/models/model_rmnist_6aug6_Ep100000_p1.pth',
            r'/home/research/projects2/iterative-semiotics-networks/models/model_rmnist_7aug6_Ep100000_p1.pth',
            r'/home/research/projects2/iterative-semiotics-networks/models/model_rmnist_8aug6_Ep100000_p1.pth',
            r'/home/research/projects2/iterative-semiotics-networks/models/model_rmnist_9aug6_Ep100000_p1.pth',
            r'/home/research/projects2/iterative-semiotics-networks/models/model_rmnist_10_aug6_Ep100000_p1.pth',
            r'/home/research/projects2/iterative-semiotics-networks/models/model_rmnist_20_aug6_Ep100000_p1.pth',
            r'/home/research/projects2/iterative-semiotics-networks/models/model_rmnist_30_aug6_Ep100000_p1.pth',
            r'/home/research/projects2/iterative-semiotics-networks/models/model_rmnist_40_aug6_Ep100000_p1.pth',
            r'/home/research/projects2/iterative-semiotics-networks/models/model_rmnist_50_aug6_Ep100000_p1.pth',
        ]
...
if dbg14:
    ...
    if tr_data_mt and not all_train_in_mt_mode:
        tr_data_mt_models = [tr_data_mt_models[0], tr_data_mt_models[1]]

if make_joint_examples__test:
    eu_op_db_spec_s = [eu_op_db_spec_s[3]]
    model_fn_op2op3_s = [model_fn_op2op3_s[3]]
...
if make_joint_examples__test:
    assert not conntype_4_make_joint_examples__test == 'stoch' or (dbg15 == 1 and hconn_simulatenonhconn == 0)
    assert not conntype_4_make_joint_examples__test == 'vanilla' or (dbg15 == 0 and hconn_simulatenonhconn == 1)
...
assert not make_joint_examples__train or eu_op == 'op2'
assert not make_joint_examples__test or eu_op == 'op3'
assert not make_joint_examples__train or op_batch == 1
assert not make_joint_examples__test or op_batch == 1
assert not (make_joint_examples__train and make_joint_examples__test)
        ...
        if train_data_mnist == 0:
            ...
            if tmptmp:
                for images, labels in my_dataloader:
                    filenames = [sample[0] for sample in my_dataloader.dataset.samples]
                    print(filenames)
                    print (labels)
            ...
            if blk:
                if skip:
                    ...
                    if h2:
                        x = self.conv5(x)
                        x1 = cosid(x)
                        dig_out = self.log_softmax(x, dim = 1)  # torch.Size([8, 10, 1, 1])
...
def map_examples_to_attractors(n_run, eu_op_loc):  # lab_hgkfdsjgtrklsg
    ...
    for nb, (ims, labs) in enumerate(glb.data_feed_autoenc):  # torch.Size([8, 1, 28, 28])
        ...
        elif make_joint_examples__test:
            # d0a(t2i(ims[0]))
            tmp=10
            #gt4_MO4_MS4_11

            #suffix_joint_exs: eg  'gt5_MO0_MS5_01',
            # see get_info_for_save() for description
            save_joint_exs, suffix_joint_exs = get_info_for_save( \
                    conntype_4_make_joint_examples__test,\
                    nimabs, data__make_examples, am_per_case,
                    list_nimabs_vanilla = list_nimabs_vanilla)

            if zoomin_nimabs:
                if zoomin_nimabs == nimabs:
                    pass
                else:
                    nimabs +=1
                    continue
                ...
                if save_joint_exs:
                    assert 'gt' + str(labs[0].numpy()) + '_MO' in suffix_joint_exs
                    fn0 = 'nimabs=' + str(nimabs) + '_' + suffix_joint_exs + '_obs' + '.png'
                    fn = os.path.join(di_examples, fn0)
                    tv.io.write_png(i2t(i2d(t2i(ims[0]))), fn)
                    print('image written to ' + str(fn))
                else:
                    nimabs +=1
                    continue
        ...
        if op_batch:
            if make_test_data_folder:
                ...
                for nstep_p1 in range(nsteps_p1):
                    print('nimabs=' + str(nimabs) + \
                            ' n_run=' + str(n_run) + ' nb=' + str(nb) + ' of ' + str(
                    ...
                    if nstep_p1 < k_h:
                        ...
                        if zoomin_nimabs:
                            ...
                            fig, ax = plt.subplots()
                            grid_image = imgaug.draw_grid(micro_batch_processed.images_aug[0:20], rows=4, cols=5)
                            ax.imshow(grid_image, cmap='gray')

                            # Remove axes and labels
                            ax.axis('off')

                            plt.savefig('grid_figure' + '_nimabs=' + str(nimabs) +  \
                                    '_nstep_p1=' + str(nstep_p1) + '.png', dpi=100, \
            ...
            if save_joint_exs:
                fn0 = 'nimabs=' + str(nimabs) + '_' + suffix_joint_exs + '_seen' + '.png'
                fn = os.path.join(di_examples, fn0)
                tv.io.write_png(i2t(i2d(t2i(ims[0]))), fn)
                print('image written to ' + str(fn))
...
def main_run():
    ...
    for n_run, eu_op_db_spec in enumerate(eu_op_db_spec_s):
        ...
        if normal_run:
            ...
            if make_joint_examples__test:
                sys.exit('normal ternimation at make_joint_examples__test, \
                        data written to '+str(di_examples))
